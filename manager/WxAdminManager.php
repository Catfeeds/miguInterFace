 ...
 * Time: 20:24
 */
class WxAdminManager extends WxAdmin{

    public static function getNickName($str){
        if(empty($str)) return '未知错误';
        if(is_numeric($str)){
            $admin = WxAdmin::model()->findByPk($str);
        }else{
    ...
    public static function getAdmin($name){
        ...
        return WxAdmin::model()->find($criteria);
    }
    ...
    public static function getAdminList($data){
        ...
        $sql_from = ' from yd_wx_admin as a inner join yd_wx_group as g on g.id=a.auth';
