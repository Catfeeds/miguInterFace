 ...
 * Time: 20:24
 */
class MvAdminManager extends MvAdmin{

    public static function getNickName($str){
        if(empty($str)) return '未知错误';
        if(is_numeric($str)){
            $admin = MvAdmin::model()->findByPk($str);
        }else{
    ...
    public static function getAdmin($name){
        ...
        return MvAdmin::model()->find($criteria);
    }
    ...
    public static function getAdminList($data){
        ...
        $sql_from = ' from yd_mv_admin as a inner join yd_mv_group as g on g.id=a.auth';
