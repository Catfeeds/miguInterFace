...
class GroupController extends MController{
	public function actionIndex(){
		...
		$criteria->select = '*';
		$count = MvGroup::model()->count($criteria);
		$criteria->offset = $data['start'];
		$criteria->limit = $data['limit'];
		$list = MvGroup::model()->findAll($criteria);
	...
	public function actionAdd(){
		try{
			if(!empty($_GET['id']) && is_numeric($_GET['id'])){
				$group = MvGroup::model()->findByPk($_GET['id']);
...
//				}
		...
		$auth = MvGuideManager::getList();

        $fulei = MvGuide::model()->findAll("pid=0 order by `order` desc");
	...
	public function actionDel(){
		...
		$del = MvGroup::model()->deleteByPk($_POST['id']);
		if(!$del){
