...
class AppController extends WController{
    ...
    public function actionIndex(){
        ...
        /*$tmp = WxSeapp::Model()->findAll("type='$type' and gid='$gid' order by orders asc");
        $app=array();
        if(!empty($tmp)){
        ...
        $sql = "select s.id,m.imageUrl,s.orders from yd_wx_seapp s,yd_wx_app m where s.gid=$gid and m.type='$type' and s.aid=m.id order by s.orders asc";
        $app = Yii::app()->db->createCommand($sql)->queryAll();
    ...
    public function actionApp(){
        ...
        if(!empty($_REQUEST['word'])){
            $word = $_REQUEST['word'];
            $tmp = WxApp::Model()->findAll("name like '%$word%' and type='$type' order by addTime desc");
        }else{
            $tmp = WxApp::Model()->findAll("type='$type' order by id desc");
        }
    ...
    public function actionDetail(){
        ...
        if(!empty($_REQUEST['word'])){
            $word = $_REQUEST['word'];
            $tmp = WxApp::Model()->findAll("name like '%$word%' order by addTime desc");
        }else{
            $tmp = WxApp::Model()->findAll(array('order'=>'addTime desc'));
        }
        $app=array();
        ...
        try{
            ...
            if(!empty($_POST)){
                ...
                if(!empty($_FILES['imgs']['tmp_name'])){
                    ...
                    $app -> imageUrl = 'http://portalpic.itv.cmvideo.cn:8088/file/' . $path;
