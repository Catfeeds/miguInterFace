...
class AdminController extends WController{
    ...
    public function actionIndex(){
        ...
        $admin = WxAdminManager::getAdminList($data);
    ...
    public function actionAdd(){
        try{
            if(empty($_GET['id'])){//如果能够接收到id  则实行添加功能
                $admin = new WxAdmin();//实例化admin
                $admin->addTime = time();//因为是添加，所以这里网数据库添加字段的信息是添加信息时间
            }else{
                $admin = WxAdmin::model()->findByPk($_GET['id']);//根据接收到的id 查询出要修改的那条数据
            ...
            if(!empty($_POST)){//如果接收的表单数据不为空
                ...
                if($admin->isNewRecord && WxAdmin::model()->exists('username=:user',array(':user'=>$post['user']))){
                    throw new ExceptionEx('登陆账号已存在');
                }
                if($admin->isNewRecord && WxAdmin::model()->exists('email=:email',array(':email'=>$post['email']))){
    ...
    public function actionDetail(){
        ...
        $group = WxGroup::model()->findAll();
        $admin = WxAdmin::model()->findByPk($_GET['id']);
    ...
    public function actionDell(){
        ...
        $del = WxAdmin::model()->findByPk($_POST['id']);
        if(empty($del)){
    ...
    public function actionDel(){
        ...
        $del = WxAdmin::model()->deleteByPk($_POST['id']);
        if(!$del){
            $this->die_json(array('code'=>404,'msg'=>'删除失败'));
        }
