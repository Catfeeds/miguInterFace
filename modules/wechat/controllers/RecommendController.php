...
class RecommendController extends WController{
    ...
    public function actionAdd(){
        ...
        $classify = !empty($_REQUEST['classify'])?$_REQUEST['classify']:'';
        if($classify=='蓝光'){
           $classify='电影';
        }
        $page = 20;
        $data = $this->getPageInfo($page);
        $list = WxMovManager::getLists($data,$classify,$cp,$word);
    ...
    public function actionVideo(){
        ...
        $sql = "select s.id,m.img,s.orders from yd_wx_select s,yd_wx_movie m where s.gid=$gid and s.mid=m.id order by s.orders desc";
        $video = Yii::app()->db->createCommand($sql)->queryAll();
        //var_dump($video);die;
    ...
    public function actionSearch(){
        ...
        $tmp = WxMovManager::getAlls($cp,$type,$word,$p);
        foreach($tmp as $key=>$val){
            switch($val['cp']) {
                case '1':
                    $val['cp'] = "华数";
