...
<div  class="content" style="height:550px;overflow: auto">
    <div style="float:left;width:190px">
        ...
        <button class="btnall bbtn">全选</button>
        <button class="btnno bbtn">全不选</button>
        <div class="fenlei">
            <ul>
                <li>按牌照方</li>
                <li class="cp-1">华数</li>
...
<script>
    ...
    $('.fenlei ul li').click(function(){
        ...
        var p = 1;
        var classify = "<?php echo !empty($_REQUEST['classify'])?$_REQUEST['classify']:''?>";
        window.location.href = "/wechat/recommend/addlist.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&classify="+classify+"&cp="+cp+"&page="+p+"&word="+word;
        //window.location.href = "/wechat/recommend/addlist.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&classify=<?php echo $_GET['classify']?>&cp="+cp+"&page="+p+"&word="+word;
    ...
    $('.classify').click(function(){
        ...
        var word = "<?php echo !empty($_REQUEST['word'])?$_REQUEST['word']:''?>";
        window.location.href = "/wechat/recommend/addlist.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&word="+word+"&cp="+cp+"&classify="+classify;
    })
    ...
    $('.btn').click(function(){
        ...
        if(ids==''){
            ...
            $.post('/wechat/recommend/doadd?mid=<?php echo $this->mid?>&nid=<?php echo $nid?>&classify=<?php echo $_REQUEST['classify']?>',{ids:ids},function(data){
                window.location.href="<?php echo Yii::app()->createUrl('/wechat/recommend/videolist',array('mid'=>$this->mid,'nid'=>$_GET['nid'],'classify'=>$_REQUEST['classify']))?>";
            })
        }
