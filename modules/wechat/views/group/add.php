        ...
        <tr>
            ...
            <td id="authList">
                ...
                <div class="hr">
                    ...
                    <?php
                    $id=$v['id'];
                    $zilei = WxGuide::model()->findAll("pid=$id order by `order` asc");
                    foreach($zilei as $key=>$val){?>
                        ...
                        <?php
                        $id=$val['id'];
                        $erjilei = WxGuide::model()->findAll("pid=$id order by `order` asc");
                        foreach($erjilei as $keys=>$vals){?>
                            <label for="auth<?php echo $vals['id']?>">
                                <input type="checkbox" value="<?php echo $vals['id']?>" <?php echo in_array($vals['id'],$auths)?'checked="checked"':''?> name="auth[]" id="auth<?php echo $vals['id']?>" class="ziji<?php echo $val['id'];?>"  rel="<?php echo $val['id'];?>"  onclick="ziji(this);"> <?php echo $vals['name']?>
                            </label>
                            <br /><span style="width:100px;display: inline-block;"></span>
                            <?php
                            $id=$vals['id'];
                            $sanlei = WxGuide::model()->findAll("pid=$id order by `order` asc");
                            foreach($sanlei as $key=>$val){?>
                                <label for="auth<?php echo $val['id']?>">
                                    <input type="checkbox" value="<?php echo $val['id']?>" <?php echo in_array($val['id'],$auths)?'checked="checked"':''?> name="auth[]" id="auth<?php echo $val['id']?>" class="ziji<?php echo $v['id'];?>"  rel="<?php echo $v['id'];?>"  onclick="ziji(this);"> <?php echo $val['name']?>
                                </label>
                                <br /><span style="width:100px;display: inline-block;"></span>
