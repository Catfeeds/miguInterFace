...
<script>
    $('.btn').click(function(){
        ...
        if(ids==''){
            ...
            $.post('/wechat/app/appadd?mid=<?php echo $this->mid?>&nid=<?php echo $_REQUEST['nid']?>&classify=<?php echo $_REQUEST['classify']?>',{ids:ids},function(d){
                window.location.href="<?php echo Yii::app()->createUrl('/wechat/app/index',array('mid'=>$this->mid,'nid'=>$_GET['nid'],'classify'=>$_REQUEST['classify']))?>";
            })
        }
    ...
    $('.search_btn').click(function(){
        var word = $('.search').val();
        window.location.href="/wechat/app/app?mid=<?php echo $_GET['mid']?>&nid=<?php echo $_GET['nid']?>&classify=<?php echo $_GET['classify']?>&word="+word
    })
