<form downloadUrl="" method="post" enctype="multipart/form-data">
    <table class="mtable" cellpadding="10" cellspacing="0" width="100%">
        ...
        <tr>
            ...
            <td>
                <select name="type" id="type" class="form-input w200">
                    ...
                    <option <?php echo !empty($app->type) && $app->type =="教育"?'selected="selected"':''?> value="教育">教育</option>
                    <option <?php echo !empty($app->type) && $app->type =="娱乐"?'selected="selected"':''?> value="娱乐">娱乐</option>
        ...
        <tr>
            ...
            <td>
                <textarea cols="55" name="description" id="description" style="border: 1px solid #ccc;border-radius: 3px;height: 100px;box-shadow: 1px 1px 8px #dddddd inset;"><?php echo !empty($app->description)?$app->description:''?></textarea>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                <input name="imgss" type="text" id="imgss" class="form-input w400" value="<?php echo !empty($app->imageUrl)?$app->imageUrl:''?>">
            </td>
        </tr>
        <tr>
            <td>&nbsp;</td>
...
<script>
    $('#appid').blur(function(){
        ...
        $('#name').val(d['name']);
            ...
            $('#creatorName').val(d['creatorName']);
            $('#size').val(d['size']);
            $('#version').val(d['name']);
            $('#description').val(d['description']);
            $('#imgss').val(d['imageUrl']);
        })
    })
</script>
