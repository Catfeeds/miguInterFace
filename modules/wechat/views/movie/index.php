...
//        },'json');
        var title = "<?php echo !empty($_REQUEST['title'])?$_REQUEST['title']:''?>";
        var classify = "<?php echo !empty($_REQUEST['classify'])?$_REQUEST['classify']:''?>";
        window.location.href = "/wechat/movie/index.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&cp="+i+"&classify="+classify+"&title="+title;
    }
    function search(){
        var title = $("#search").val();
        var classify = "<?php echo !empty($_REQUEST['classify'])?$_REQUEST['classify']:''?>";
        var cp = "<?php echo !empty($_REQUEST['cp'])?$_REQUEST['cp']:''?>";
        window.location.href = "/wechat/movie/index.html?mid=<?php echo $this->mid?>&nid=<?php echo $_GET['nid']?>&title="+title+"&cp="+cp+"&classify="+classify;
    }
</script>
<body>
<div style="width: 200px;overflow: hidden;float: left;clear: both">
    <div>
        <input type="text" name="select" id="search" class="form-input w100" placeholder="输入片名搜索" value="<?php echo !empty($_REQUEST['title'])?$_REQUEST['title']:''?>" onFocus="if(value==defaultValue){value='';this.style.color='#000'}" onBlur="if(!value){value=defaultValue;this.style.color='#999'}" style="color:#999999">
    ...
    <div class="bttn" style="margin-top: 10px">
        <button class="btn1 classify 电影" >电影</button><br />
        <button class="btn2 classify 电视剧" >电视剧</button><br />
        <button class="btn3 classify 综艺">综艺</button><br />
        <button class="btn4 classify 少儿">少儿</button><br />
        <button class="btn5 classify 甘肃蓝光" >甘肃蓝光</button><br />
        <button class="btn6 classify 甘肃电影" >甘肃电影</button><br />
        <button class="btn7 classify 甘肃电视剧" >甘肃电视剧</button><br />
        <button class="btn8 classify 少儿多集" >少儿多集</button><br />
    </div>
</div>
<div style="margin-left: 200px;">
    ...
    <div>
        ...
        <div>
            <?php echo $page;?>
            <div style="width:250px;position:relative;float:right;top:-60px;right:-9rem">
                <input class="form-input w100 pagenum" value="" style="width:30px">
                <button class="btn go" style="padding:8px 10px">GO！</button>
            </div>
        </div>
    </div>
</div>
