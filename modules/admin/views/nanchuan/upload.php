...
<table class="mtable" width="100%" cellspacing="0" cellpadding="10">
    <tr>
        ...
        <td>
            <select name="type" onchange="aa()" class="form-input w300" id="type" onchange="aa()">
                <option value="0">请选择</option>
                <option <?php if($type==1){echo "selected=selected"; } ?> value="1" >图片</option>
                <option <?php if($type==2){echo "selected=selected"; } ?> value="2">视频</option>
                <option <?php if($type==3){echo "selected=selected"; } ?> value="3">应用</option>
                <option <?php if($type==4){echo "selected=selected"; } ?> value="4">自有节目</option>
            </select>
        </td>
    </tr>
    <tr>
        <td width="100" align="right">标题：</td>
        <td><input type="text" name="title" id="title" value="<?php echo !empty($nanchuan[$address][$fid]['title'])?$nanchuan[$address][$fid]['title']:''?>" class="form-input"></td>
    </tr>
    <tr>
        <td align="right">url：</td>
        <td><input type="text" name="url" id="url" value="<?php echo !empty($nanchuan[$address][$fid]['url'])?$nanchuan[$address][$fid]['url']:''?>" class="form-input">
            <input type="text" readonly class="form-input" id="yingyong" style="display: none" value="例如：?viewType=2&appId=201405151707249">
            <input type="text" readonly class="form-input" id="show" style="display: none" value="例如：http://hn.voolepg.itv.cmvideo.cn/filmInfo/mid.action?page=1&mid=45748745&mtype=0&auth=1">
        </td>
    </tr>
    <tr>
...
<input type="hidden" name="key" value="<?php echo !empty($nanchuan[$address][$fid]['pic'])?$nanchuan[$address][$fid]['pic']:''?>">
<input type="hidden" name="pos" value="<?php echo $address;?>">
<input type="hidden" name="id" value="<?php echo !empty($nanchuan[$address][$fid]['id'])?$nanchuan[$address][$fid]['id']:''?>">
<script type="text/javascript">
    ...
    $('.save').click(function(){
        ...
        G.title = $('#title').val();
        G.url = $('#url').val();
        G.pos = $('input[name=pos]').val();
        G.key = $('input[name=key]').val();
        G.type = $('select[name=type]').val();
        ...
        $.post('/admin/nanchuan/add?mid=<?php echo $this->mid ?>',G,function(d){
            if(d.code == 200){
                location.reload();
            }else{
                layer.close(load);
