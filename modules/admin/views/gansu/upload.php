...
<table class="mtable" width="100%" cellspacing="0" cellpadding="10">
    <tr>
        ...
        <td>
            <select name="tType"  class="form-input w300" id="tType" >
...
<script type="text/javascript">
    ...
    $('.save').click(function(){
        ...
        if(empty(G.pos)){
            layer.alert('1系统错误',{icon:0});
            return false;
        }

        if(empty(G.key)){
            layer.alert('2系统错误',{icon:0});
            return false;
        }
        if(empty(G.type)){

            layer.alert('3系统错误',{icon:0});
            return false;
        }
        if(empty(G.cp)){
            layer.alert('4系统错误',{icon:0});
            return false;
        }
        G.provinceCode = $('input[name=provinceCode]').val();
        G.cityCode = $('input[name=cityCode]').val();
        var load = layer.load(0, {icon: 16,shade: [0.3,'#000']});
        $.post('/admin/gansu/add?mid=<?php echo $this->mid?>',G,function(d){
            if(d.code == 200){
                location.reload();
            }else{
                layer.close(load);
