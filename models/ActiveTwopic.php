...
/**
 * This is the model class for table "{{active_twopic}}".
 *
 * The followings are the available columns in table '{{active_twopic}}':
 * @property string $id
 * @property string $uid
 ...
 */
class ActiveTwopic extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('uid, type, province, city, cp, pos, epg, cid, cTime, vname', 'required'),
			array('uid, rand', 'length', 'max'=>10),
			array('type, cp, pos', 'length', 'max'=>20),
			...
			array('id, uid, type, province, city, cp, pos, epg, cid, cTime, vname, title, rand, usergroup, epgcode', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return ActiveTwopic the static model class
	 */
