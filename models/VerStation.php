...
class VerStation extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('delFalg, template, guide, message, circular', 'numerical', 'integerOnly'=>true),
			array('gid', 'length', 'max'=>11),
			array('province, city', 'length', 'max'=>40),
			array('cp, cps', 'length', 'max'=>50),
			array('usergroup, epgcode, logo', 'length', 'max'=>255),
			array('mark, name', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, gid, province, city, cp, usergroup, epgcode, delFalg, mark, name, logo, cps, template, guide, message, circular', 'safe', 'on'=>'search'),
		);
	}
